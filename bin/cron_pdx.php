...
$shizzowup = 'shizzeepspdx:' . PDXPASS;

// get places
$url = 'https://v0.api.shizzow.com/places?populated=true&cities=Portland&states=OR&limit=' . $limit;
...
try{
	...
	// delete old data
	$sql = "DELETE FROM Storage WHERE City = 'pdx';";
	$db->query($sql);
	
	// add the new data
	$escaped = mysql_real_escape_string  ($serialized);
	$sql = "INSERT INTO Storage (Data, City) VALUES ('$escaped', 'pdx')";
	//echo "<hr/>$sql<hr/>";
	$db->query($sql);
	$db = null;	
}
