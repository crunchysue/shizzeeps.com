<?php

// Rename this file to lucy.php and put in your own usernames and passwords and stuffs.

define ('TWITTERUP', 'username:password');
define ('SHIZZOWUP', 'username:password');
define ('DBHOST', 'mydbhost');
define ('DBUSER', 'mydbusername');
define ('DBPASS', 'mydbpassword');
define ('DBNAME', 'mydbname');
define ('ERR_EMAIL', 'you@yours.com');
define ('TWEET_CRON_PASS', 'mypassword');
define ('FIND_SHIZZEEPS_CRON_PASS', 'mypassword');
define ('PDXPASS', 'mypassword');
define ('AUSPASS', 'mypassword');
define ('RSSPASS', 'mypassword');


?>